...
function buildUserPrompt(type, q, ctx) {
...
function generateFast(req, res, { type, q, slug, lan, ctx }) {
  (lan ? lanGate : genGate).run(() => new Promise(resolve => {
    ...
    (async () => {
      try {
        ...
        const { html, issues } = await producePage(s, res, { system, userPrompt: buildUserPrompt(type, q, ctx) });
        if (s.aborted) return;
...
const server = http.createServer((req, res) => {
  ...
  if (u.pathname === '/api/generate') {
    ...
    return generateFast(req, res, { type, q, slug: type === 'search' ? slugify(q) : null, lan, ctx });
  }
